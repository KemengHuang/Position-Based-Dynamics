...
void update_cloth_edges() {
	...
	for (int i = 0; i < cloth.positions.size(); i++) {
		cloth.velocities[i] = cloth.velocities[i] + dt * gravity_force;
		cloth.predict_positions[i] = cloth.positions[i] + cloth.velocities[i] * dt;
	}
	...
	for (int k = 0; k < 10; k++) {
		...
		for (int i = 0; i < cloth.edges.size(); i++) {
			...
			double lambda_min = 0.9* cloth.lengths[i];
			double lambda_max = 1.1* cloth.lengths[i];
			double current_length = (point0 - point1).norm();
			double clamped_length = min(max(lambda_min, current_length), lambda_max);
			double scale = clamped_length / current_length;
			double mass0 = cloth.masses[cloth.edges[i][0]];
			double mass1 = cloth.masses[cloth.edges[i][1]];

			Vector3d bary_center = (1. / (mass0 + mass1)) * (mass0 * point0 + mass1 * point1);
			Vector3d new_point0 = bary_center + scale * (point0 - bary_center);
			Vector3d new_point1 = bary_center + scale * (point1 - bary_center);

			cloth.predict_positions[cloth.edges[i][0]] = new_point0;
			cloth.predict_positions[cloth.edges[i][1]] = new_point1;
		}
	...
	for (int i = 0; i < cloth.positions.size(); i++) {
		cloth.velocities[i] = (cloth.predict_positions[i] - cloth.positions[i]) / dt;
		cloth.positions[i] = cloth.predict_positions[i];
	}
}
